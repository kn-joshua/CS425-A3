...
bool receive_syn_ack(int sock) {
    ...
    //time_t start_time = time(NULL);  //Current system time
    //const int time_out = 5; // Time out duration in seconds
    struct timeval tv;
    tv.tv_sec = 5;   //Time out in seconds
    tv.tv_usec = 0;  //No microsecond component
    ...
    while (true) {
	    ...
	    if(errno == EAGAIN || errno == EWOULDBLOCK) {
		    std::cerr << "Timeout: No SYN-ACK recieved. Handshake failed" << std::endl;
	    	    return false;
	    }
        ...
        if (tcp->syn == 1 && tcp->ack == 1) {
		    ...
		    std::cout<<"Recieved incorrect sequence/ack number from server. Handshake failed"<<std::endl;
		    return false;
        	}
	}
    }
